...
function changeRoomManagerPrice(roomId) {var date = $('#newRoomManagerPriceDate' + roomId).val();$.ajax({url: '/ajax/set_manager_price', type: 'POST', data: {roomId: roomId, firstDate: date, lastDate: date, price: $('#newRoomManagerPrice' + roomId).val()}, async: true}).done(function () {alert('Изменение стоимости бронировании успешно завершено!');location.reload();});}