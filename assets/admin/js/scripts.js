...
function showBookingPeriod(id, status) {var colors = {'success' : '#82c44f' , 'canceled' : '#bfe1b2', 'active' : '#ecd20a', 'occupancy' : '#3c763d'};var $elems = $('.booking-ceil-period-' + id);var background = rgbToHex($elems.css('background-color'));if(background === '#00d7ec') {$elems.css('background-color', colors[status]);} else {$elems.css('background-color', '#00d7ec');}}
...
function canceledBooking(bookingId) {$.ajax({url: '/ajax/canceled_booking', type: 'POST', data: {reservationId: bookingId}, async: true}).done(function () {location.reload();});}
function dayCanceledBooking(bookingId, year, month, day) {$.ajax({url: '/ajax/day_canceled_booking', type: 'POST', data: {bookingId: bookingId, date: year + '-' + month + '-' + day}, async: true}).done(function () {location.reload();});}
...
function changeBooking(bookingId, roomId) {var paymentStatus = parseInt($('#inputChangePaymentStatus').val());var paymentValue = $('#inputAddPayment').val();if((paymentValue == 0 || paymentValue == '') && (paymentStatus === 2 || paymentStatus === 3)){alert('Укажите сумму оплаты!');return;}var dates = getDates('inputChangePeriod');$.ajax({url: '/ajax/check_room_reserve', type: 'POST', data: {bookingId: bookingId, roomId: roomId, arrivalDate: dates[0], departureDate: dates[1]}, async: true}).done(function (response) {if(response === 'free') {$.ajax({url: '/ajax/change_booking', type: 'POST', data: {bookingId: bookingId, roomId: roomId, phone: $('#inputChangePhone').val(), name: $('#inputChangeName').val(), comment: $('#inputChangeComment').val(),arrivalDate: dates[0], departureDate: dates[1], adult: $('#inputChangeAdult').val(), childrenTo2: $('#inputChangeChildrenTo2').val(), childrenTo6: $('#inputChangeChildrenTo6').val(), childrenTo12: $('#inputChangeChildrenTo12').val(), price: $('#inputChangePrice').val(), type: $('#inputChangeType').val(), status: $('#inputChangeStatus').val(), paymentStatus: paymentStatus, paymentValue: paymentValue}, async: true}).done(function () {alert('Изменение данных о бронировании успешно завершено!');location.reload();});}else{alert('Указанный период бронирования недоступен!');}});}
function changeRoomPrice(roomId) {var date = $('#newRoomPriceDate' + roomId).val();var $price = $('#newRoomPrice' + roomId);$.ajax({url: '/ajax/set_price', type: 'POST', data: {roomId: roomId, firstDate: date, lastDate: date, price: $price.val()}, async: true}).done(function () {alert('Изменение стоимости бронировании успешно завершено!');$('#bookingPriceChange_' + roomId + '_' + date).html($price.val());if($('#bookingManagerPrice' + roomId + '_' + date).val() == 0){$('.manager-price-' + roomId + '-' + date + ' span').html($price.val());}$('.popover').remove();});}
function changeRoomManagerPrice(roomId) {var date = $('#newRoomManagerPriceDate' + roomId).val();var $price = $('#newRoomManagerPrice' + roomId);$.ajax({url: '/ajax/set_manager_price', type: 'POST', data: {roomId: roomId, firstDate: date, lastDate: date, price: $price.val()}, async: true}).done(function () {alert('Изменение стоимости бронировании успешно завершено!');$('#bookingManagerPriceChange_' + roomId + '_' + date).html($price.val());$('.popover').remove();});}