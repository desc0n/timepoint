...
function showBookingDetails(roomId) {var $cells = $('.booking-hidden-' + roomId);var checkDisplayStyle = $cells.css('display');if(checkDisplayStyle === 'none'){$cells.css('display', 'table-cell');} else {$cells.css('display', 'none');}}
...
function changeBooking(bookingId, roomId) {
    var dates = getDates('inputChangePeriod');$.ajax({url: '/ajax/check_room_reserve', type: 'POST', data: {bookingId: bookingId, roomId: roomId, arrivalDate: dates[0], departureDate: dates[1]}, async: true}).done(function (response) {if(response === 'free') {$.ajax({url: '/ajax/change_booking', type: 'POST', data: {bookingId: bookingId, roomId: roomId, phone: $('#inputChangePhone').val(), name: $('#inputChangeName').val(), comment: $('#inputChangeComment').val(),arrivalDate: dates[0], departureDate: dates[1], adult: $('#inputChangeAdult').val(), childrenTo2: $('#inputChangeChildrenTo2').val(), childrenTo6: $('#inputChangeChildrenTo6').val(), childrenTo12: $('#inputChangeChildrenTo12').val(), price: $('#inputChangePrice').val()}, async: true}).done(function () {alert('Изменение данных о бронировании успешно завершено!');location.reload();});}else{alert('Указанный период бронирования недоступен!');}});
}