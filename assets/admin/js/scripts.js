...
function reserveRoom() {
    var dates = getDates();$.ajax({url: '/ajax/check_room_reserve', type: 'POST', data: {roomId: $('#inputRoom').val(), arrivalDate: dates[0], departureDate: dates[1]}, async: true}).done(function (response) {if(response === 'free') {$.ajax({url: '/ajax/reserve_room', type: 'POST', data: {roomId: $('#inputRoom').val(), phone: $('#inputPhone').val(), name: $('#inputName').val(), comment: $('#inputComment').val(),arrivalDate: dates[0], departureDate: dates[1], adult: $('#inputAdult').val(), childrenTo2: $('#inputChildrenTo2').val(), childrenTo6: $('#inputChildrenTo6').val(), childrenTo12: $('#inputChildrenTo12').val(), type: $('#inputType').val(), price: $('#inputPrice').val()}, async: true}).done(function () {alert('Номер успешно забронирован!');location.reload();});}else{alert('Бронирование номера не доступно!');}});
}
